...
class OrderTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(username="user1", email="user1@user", password="user123")
        ...
        pizzaSicilian = Pizza.objects.create(base=sicilianBase, toppings=special, small=30.0, large=40.0)
        pizzaRegular = Pizza.objects.create(base=regularBase, toppings=special, small=20.0, large=30.0)
        ...
        sub = Sub.objects.create(ingredients="Meatballs", small=6.00, large=9.00)
        ...
        dinner = DinnerPlate.objects.create(ingredients="antipasto", small=40.00, large=60.00)
    ...
    def test_login(self):
        response = self.client.post('/login/', {'username': 'user1', 'password': 'user123'})
    ...
    def test_orderPizza(self):
        ...
        response = self.client.post('/pizza/', {"sicilian_toppings": "Special", "size" : 'small', "num" : 3})
        items = OrderItem.objects.filter(order=self.order)
        self.assertEqual(items.count(), 2)
        self.assertEqual(response.status_code, 200)
        response = self.client.post('/pizza/', {"regular_toppings": "Special", "size" : 'large', "num" : 1})
        self.assertEqual(items.count(), 3)
        self.assertEqual(response.status_code, 200)

    def test_orderSub(self):
        self.client.login(username='user1', password='user123')
        response = self.client.post('/Sub/', {"sub": "Meatballs", "size" : 'small', "num" : 1, "extra" : "none"})
        items = OrderItem.objects.filter(order=self.order)
        self.assertEqual(items.count(), 2)
        self.assertEqual(response.status_code, 200)
    ...
    def test_orderPasta(self):
        self.client.login(username='user1', password='user123')
        response = self.client.post('/Pasta/', {"pasta": "Baked Zitti", "num" : 1})
        items = OrderItem.objects.filter(order=self.order)
        self.assertEqual(items.count(), 2)
        self.assertEqual(response.status_code, 200)
    ...
    def test_orderSalad(self):
        self.client.login(username='user1', password='user123')
        response = self.client.post('/Salad/', {"salad": "garden salad", "num" : 1})
        items = OrderItem.objects.filter(order=self.order)
        self.assertEqual(items.count(), 2)
        self.assertEqual(response.status_code, 200)
    ...
    def test_orderDinner(self):
        self.client.login(username='user1', password='user123')
        response = self.client.post('/Dinner/', {"dinner": "antipasto", "size" : "large" ,"num" : 1})
        items = OrderItem.objects.filter(order=self.order)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(items.count(), 2)
